...
<div id="content_js" style="display:none;">
    ...
    head: [['N°',  'Désignation', 'UM/Cond', 'Qté', 'Prix U HT', 'Total HT']],
    ...
    <%
        if (!data.get("timbre").equals("")) {
    %>
    <% // if (Double.parseDouble(data.get("timbre")) > 0) {%>
    ['Timbre 1%',' <%= data.get("timbre")%>'],
    <% // }%>
    <%  
        }
    %>
    ...
    foot: [
    <% if (data.get("cond1").equals("non") && data.get("cond2").equals("non") && data.get("cond3").equals("non") && data.get("cond4").equals("non") && data.get("cond5").equals("non")) {%>

    ['','EURL IMTRADE'],
    <% } else {%>
    ['Conditions réglementaires :','EURL IMTRADE'],
    <% }%>
    <% if (!data.get("cond1").equals("non")) {%>
    ['-  Les prix ne sont valides que pour la totalité de l\'offre.',''],
    <% }%>
    <% if (!data.get("cond2").equals("non")) {%>
    ['-  Mode de règlement:  <%=data.get("cond2")%>',''],
    <% }%>
    <% if (!data.get("cond3").equals("non")) {%>
    ['-  Mode de livraison:  <%=data.get("cond3")%>',''],
    <% }%>
    <% if (!data.get("cond4").equals("non")) {%>
    ['-  Délais de livraison:  <%=data.get("cond4")%>',''],
    <% }%>
    <% if (!data.get("cond5").equals("non")) {%>
    ['-  Validité de l\'offre:  <%=data.get("cond5")%>',''],
    <% }%>
    ],
    margin: { top: 56,bottom: 35,left:8,right:45,width: 100},
    })
